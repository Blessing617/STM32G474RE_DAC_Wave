...
void WaveStart(int wave)
{
	switch (wave)
	{
		...
		case squ1 :   HAL_TIM_Base_Start(&htim6);
					  HAL_DAC_Start_DMA(&hdac1,DAC_CHANNEL_1,(uint32_t *)squTable,tableSize,DAC_ALIGN_12B_R);
					  break;
		
		case tri :    HAL_TIM_Base_Start(&htim6);
					  HAL_DAC_Start_DMA(&hdac1,DAC_CHANNEL_1,(uint32_t *)triTable,tableSize,DAC_ALIGN_12B_R); 
					  break;
